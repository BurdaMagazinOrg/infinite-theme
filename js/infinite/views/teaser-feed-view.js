(function($, Drupal, drupalSettings, Backbone, BurdaInfinite) {
  BurdaInfinite.views.TeaserFeedView = BaseView.extend({
    ...
    initialize: function(pOptions) {
      ...
      if (this.infiniteBlockDataModel) {
        if (
          ...
          typeof ProductView !== 'undefined' &&
          this.model.get('componentType') != ProductView.COMPONENT_TYPE_SLIDER
        ) {
          this.model.set(
            'containerType',
            this.infiniteBlockDataModel
              .get('trackingContainerType')
              .toLowerCase()
          );
        }

        if (this.infiniteBlockDataModel.has('uuid')) {
          this.advancedTrackingData = TrackingManager.getAdvTrackingByUuid(
            this.infiniteBlockDataModel.has('uuid')
          );
    ...
    createModel: function() {
      ...
      this.model.set(
        'title',
        jQuery.trim(this.$el.find('.teaser__title').text())
      );
    ...
    extendDataLayerInfo: function() {
      if (this.advancedTrackingData.hasOwnProperty('page')) {
        ...
        this.model.set(
          'contentType',
          this.advancedTrackingData.page.contentType
        );
  ...
  window.TeaserFeedView =
    window.TeaserFeedView || BurdaInfinite.views.TeaserFeedView;
